...
@section('title', 'Create Shift Policy')
...
<div class="mb-8">
    ...
    <p class="text-slate-500 text-sm font-medium">Configure a new standard corporate shift template with custom timing policies and grace limits.</p>
</div>

<div class="max-w-3xl">
    <form action="{{ route('admin.shifts.store') }}" method="POST" class="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden animate-fade-in">
        @csrf
        ...
        <div class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="shift_name" id="shift_name" value="{{ old('shift_name') }}" required
                            placeholder="e.g. Regular Day Shift, Night Support" 
                ...
                <div>
                    ...
                    <select name="shift_type" id="shift_type" required onchange="toggleShiftTimings(this.value)"
                        ...
                        <option value="general" {{ old('shift_type') == 'general' ? 'selected' : '' }}>General Day Shift</option>
                        <option value="night" {{ old('shift_type') == 'night' ? 'selected' : '' }}>Night Shift (Cross Midnight)</option>
                        <option value="rotational" {{ old('shift_type') == 'rotational' ? 'selected' : '' }}>Rotational Schedule</option>
                        <option value="flexible" {{ old('shift_type') == 'flexible' ? 'selected' : '' }}>Flexible Hours (No Fixed Timings)</option>
                    </select>
                </div>
            </div>
            ...
            <div id="shift_timings_wrapper" class="grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-300">
                <!-- Start Time -->
                <div>
                    <label for="start_time" class="block text-slate-700 font-extrabold text-xs uppercase tracking-wider mb-2">Shift Start Time (HH:MM)</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400">
                ...
                <!-- End Time -->
                <div>
                    <label for="end_time" class="block text-slate-700 font-extrabold text-xs uppercase tracking-wider mb-2">Shift End Time (HH:MM)</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="grace_time_minutes" id="grace_time_minutes" value="{{ old('grace_time_minutes', 15) }}" required min="0"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl text-sm font-semibold transition-all outline-none text-slate-800">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.5" name="half_day_time" id="half_day_time" value="{{ old('half_day_time', 4.0) }}" required min="0" max="24"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl text-sm font-semibold transition-all outline-none text-slate-800">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.5" name="minimum_working_hours" id="minimum_working_hours" value="{{ old('minimum_working_hours', 8.0) }}" required min="0" max="24"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl text-sm font-semibold transition-all outline-none text-slate-800">
                    </div>
                    ...
                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                        <label class="flex items-center gap-3.5 px-3 py-2 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 transition-colors shadow-sm cursor-pointer group">
                            <input type="checkbox" name="weekly_off_days[]" value="{{ $day }}" 
                                {{ in_array($day, old('weekly_off_days', ['Saturday', 'Sunday'])) ? 'checked' : '' }}
...
<script>
    ...
    // Trigger on page load in case of validation backflow
