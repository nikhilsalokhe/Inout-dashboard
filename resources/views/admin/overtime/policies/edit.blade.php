...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800">Edit Overtime Policy</h2>
            <p class="text-slate-500 text-sm mt-1">Update rules, thresholds, and multiplier rates.</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.overtime.policies.update', $policy) }}" method="POST" class="space-y-6">
        @csrf
        @method('POST') <!-- Should be PUT but using POST for simplicity if update route is POST. Wait, in web.php I defined update as POST. -->
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $policy->name }}" required class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Description</label>
                    <textarea name="description" rows="2" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700">{{ $policy->description }}</textarea>
                </div>
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $policy->is_active ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="rate_type" id="rateTypeSelect" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700" onchange="toggleFixedRate()">
                        <option value="salary_based" {{ $policy->rate_type === 'salary_based' ? 'selected' : '' }}>Salary Based (Hourly calculated from salary)</option>
                        <option value="fixed" {{ $policy->rate_type === 'fixed' ? 'selected' : '' }}>Fixed Flat Rate</option>
                    </select>
                </div>
                <div id="fixedRateContainer" class="{{ $policy->rate_type === 'fixed' ? '' : 'hidden' }}">
                    <label class="block text-sm font-bold text-slate-700 mb-1">Fixed Hourly Rate (₹)</label>
                    <input type="number" step="0.01" name="fixed_rate" value="{{ $policy->fixed_rate }}" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Max Payable Hours / Month</label>
                    <input type="number" step="0.5" name="max_payable_hours_per_month" value="{{ $policy->max_payable_hours_per_month }}" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700">
                </div>
            </div>
        </div>
        ...
        <div class="bg-indigo-50/50 rounded-3xl p-6 border border-indigo-100 shadow-sm">
            <div class="flex items-center justify-between mb-4 border-b border-indigo-100 pb-2">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="calc_daily" value="1" {{ $policy->calc_daily ? 'checked' : '' }} class="sr-only peer" onchange="document.getElementById('dailyOpts').classList.toggle('opacity-50')">
            ...
            <div id="dailyOpts" class="grid grid-cols-1 md:grid-cols-4 gap-4 {{ $policy->calc_daily ? '' : 'opacity-50' }} transition-opacity">
                <div>
                    <label class="block text-[11px] uppercase tracking-wider font-bold text-indigo-800 mb-1">Min Hours (Shift)</label>
                    <input type="number" step="0.5" name="daily_min_hours" value="{{ $policy->daily_min_hours }}" class="w-full px-3 py-1.5 text-sm border border-indigo-200 rounded-lg">
                </div>
                <div>
                    <label class="block text-[11px] uppercase tracking-wider font-bold text-indigo-800 mb-1">OT Threshold</label>
                    <input type="number" step="0.5" name="daily_threshold" value="{{ $policy->daily_threshold }}" class="w-full px-3 py-1.5 text-sm border border-indigo-200 rounded-lg">
                </div>
                <div>
                    <label class="block text-[11px] uppercase tracking-wider font-bold text-indigo-800 mb-1">Max OT / Day</label>
                    <input type="number" step="0.5" name="max_daily" value="{{ $policy->max_daily }}" class="w-full px-3 py-1.5 text-sm border border-indigo-200 rounded-lg">
                </div>
                <div>
                    <label class="block text-[11px] uppercase tracking-wider font-bold text-indigo-800 mb-1">Rate Multiplier</label>
                    <input type="number" step="0.01" name="daily_rate_multiplier" value="{{ $policy->daily_rate_multiplier }}" class="w-full px-3 py-1.5 text-sm border border-indigo-200 rounded-lg">
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-blue-50/50 rounded-3xl p-6 border border-blue-100 shadow-sm">
                <div class="flex items-center justify-between mb-4 border-b border-blue-100 pb-2">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="calc_weekly" value="1" {{ $policy->calc_weekly ? 'checked' : '' }} class="sr-only peer" onchange="document.getElementById('weeklyOpts').classList.toggle('opacity-50')">
                ...
                <div id="weeklyOpts" class="space-y-3 {{ $policy->calc_weekly ? '' : 'opacity-50' }}">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-blue-800">Weekly Threshold</span>
                        <input type="number" step="0.5" name="weekly_threshold" value="{{ $policy->weekly_threshold }}" class="w-24 px-2 py-1 text-sm border border-blue-200 rounded-lg">
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-blue-800">Max OT / Week</span>
                        <input type="number" step="0.5" name="max_weekly" value="{{ $policy->max_weekly }}" class="w-24 px-2 py-1 text-sm border border-blue-200 rounded-lg">
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-blue-800">Multiplier</span>
                        <input type="number" step="0.01" name="weekly_rate_multiplier" value="{{ $policy->weekly_rate_multiplier }}" class="w-24 px-2 py-1 text-sm border border-blue-200 rounded-lg">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-cyan-50/50 rounded-3xl p-6 border border-cyan-100 shadow-sm">
                <div class="flex items-center justify-between mb-4 border-b border-cyan-100 pb-2">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="calc_monthly" value="1" {{ $policy->calc_monthly ? 'checked' : '' }} class="sr-only peer" onchange="document.getElementById('monthlyOpts').classList.toggle('opacity-50')">
                ...
                <div id="monthlyOpts" class="space-y-3 {{ $policy->calc_monthly ? '' : 'opacity-50' }}">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-cyan-800">Monthly Threshold</span>
                        <input type="number" step="0.5" name="monthly_threshold" value="{{ $policy->monthly_threshold }}" class="w-24 px-2 py-1 text-sm border border-cyan-200 rounded-lg">
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-cyan-800">Max OT / Month</span>
                        <input type="number" step="0.5" name="max_monthly" value="{{ $policy->max_monthly }}" class="w-24 px-2 py-1 text-sm border border-cyan-200 rounded-lg">
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-cyan-800">Multiplier</span>
                        <input type="number" step="0.01" name="monthly_rate_multiplier" value="{{ $policy->monthly_rate_multiplier }}" class="w-24 px-2 py-1 text-sm border border-cyan-200 rounded-lg">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-rose-50/50 rounded-3xl p-6 border border-rose-100 shadow-sm">
                <div class="flex items-center justify-between mb-4 border-b border-rose-100 pb-2">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="calc_weekend" value="1" {{ $policy->calc_weekend ? 'checked' : '' }} class="sr-only peer" onchange="document.getElementById('weekendOpts').classList.toggle('opacity-50')">
                ...
                <div id="weekendOpts" class="{{ $policy->calc_weekend ? '' : 'opacity-50' }}">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-rose-800">Multiplier (e.g. 2 for 2x)</span>
                        <input type="number" step="0.01" name="weekend_rate_multiplier" value="{{ $policy->weekend_rate_multiplier }}" class="w-24 px-2 py-1 text-sm border border-rose-200 rounded-lg">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-amber-50/50 rounded-3xl p-6 border border-amber-100 shadow-sm">
                <div class="flex items-center justify-between mb-4 border-b border-amber-100 pb-2">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="calc_holiday" value="1" {{ $policy->calc_holiday ? 'checked' : '' }} class="sr-only peer" onchange="document.getElementById('holidayOpts').classList.toggle('opacity-50')">
                ...
                <div id="holidayOpts" class="{{ $policy->calc_holiday ? '' : 'opacity-50' }}">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-bold text-amber-800">Multiplier (e.g. 2 for 2x)</span>
                        <input type="number" step="0.01" name="holiday_rate_multiplier" value="{{ $policy->holiday_rate_multiplier }}" class="w-24 px-2 py-1 text-sm border border-amber-200 rounded-lg">
                    </div>
                </div>
            </div>
        </div>
